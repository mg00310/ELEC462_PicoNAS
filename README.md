...
- **전체 코드 라인 수 (C언어):** **2,449 라인**
- **핵심 기능:** 멀티스레드 서버, TUI 클라이언트, 동시 다운로드, 실시간 로깅 및 디버그 패널
- **주요 시스템 콜 및 라이브러리:** `socket`, `setsockopt`, `bind`, `listen`, `accept`, `connect`, `read`, `write`, `close`, `open`, `stat`, `lstat`, `mkdir`, `remove`, `readlink`, `realpath`, `opendir`, `readdir`, `closedir`, `fork`, `waitpid`, `signal`, `pthread_create`, `pthread_detach`, `pthread_mutex_init`, `pthread_mutex_lock`, `ncurses` 관련 함수

<br>
<small>
<b>코드 라인:</b> `cloc` 도구를 사용하여 주석/공백을 제외한 C 소스 코드(.c, .h)의 라인 수를 계산.
<br>
<b>시스템 콜:</b> `grep` 명령어로 소스 코드 내에서 직접 호출되는 고유한 시스템 콜 및 주요 라이브러리 함수명을 추출하여 집계.
</small>
